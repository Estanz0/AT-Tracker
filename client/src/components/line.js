const Line = ({ mapState: { width, height }, latLngToPixel, coordsArray, style = { stroke: 'rgb(255,0,0)', strokeWidth: 2 } }) => {
    ...
    let lines = []
    let pixel = latLngToPixel([coordsArray[0].shape_pt_lat, coordsArray[0].shape_pt_lon])
  
    for (let i = 1; i < coordsArray.length; i++) {
      let pixel2 = latLngToPixel([coordsArray[i].shape_pt_lat, coordsArray[i].shape_pt_lon])
